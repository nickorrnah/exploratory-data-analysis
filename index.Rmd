...
```{r, include=FALSE}
source("Aggregate Table Script.R", local = knitr::knit_global())
```
After consideration, only the essiential columns were kept.
...
# Peak Viewership by Month
```{r, include=FALSE}
source("bar-chart.R", local = knitr::knit_global())
```
This is a bar chart of peak viewership by month. This shows the when Twitch has the most traffic, and the purpose of this is to document Twitch's growth and expansion.

This chart clearly shows that as the years go by, Twitch is only growing and getting bigger, as the peak viewership is constantly increasing, reaching new records and surpassing the previous. The relations ship between the two axis are linear, as they both increase with each other. 
